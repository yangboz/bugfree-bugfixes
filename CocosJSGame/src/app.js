...
var UI = {
		...
		addScore:function(s){
			...
			var sp = new cc.Sprite(res.End_png);
...
var Doge = cc.Sprite.extend({
	...
	ctor:function(target){
		this._super(res.Catnorrisd_png, cc.rect(188,58,66,42));
		this.attr({
			anchorX:0.5,
			anchorY:0,
			target:target
...
var Cat = cc.Sprite.extend({
	...
	screenHeight:0,
	ctor:function(){
		this._super(res.Catnorrisd_png, cc.rect(144,148,70,32));
		this.attr({
			anchorX:0.5,
			anchorY:0
		});
...
var HelloWorldScene = cc.Scene.extend({
	...
	onEnter:function () {
		...
		var pg = new cc.Sprite(res.Pg_png);
		this.addChild(pg);
		pg.attr({
			x:230,
			y:cc.director.getVisibleSize().height - 25
